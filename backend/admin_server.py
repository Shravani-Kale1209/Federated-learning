...
async def get_admin_dashboard():
    ...
    return HTMLResponse("Admin Dashboard not found.", status_code=404)
...
async def register_hospital(data: dict):
    org_id = (data.get("org_id") or data.get("username", "")).upper()
    if not org_id:
    ...
    db[org_id] = {
        "name": data.get("hospital_name") or org_id,
        "status": "PENDING",
    ...
    JsonDB.save(db)
    print(f"[REG] Secure registration: {org_id}")
    
    # Industry-level response format
    tess_id = f"TESS-{secrets.token_hex(2).upper()}"
    return {"status": "verified", "node_id": tess_id}
...
if __name__ == "__main__":
    print("[DEBUG] Starting uvicorn on 127.0.0.1:8000...")
